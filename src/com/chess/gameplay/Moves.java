...
import com.chess.gamecontents.Bishop;
import com.chess.gamecontents.Board;
import com.chess.gamecontents.Piece;
import com.chess.gamecontents.Queen;
import com.chess.gamecontents.Rook;

/*
 * This class will handle the possible moves of Queen, Bishop and Rook as of now
 * @date - 24/08/2018
 * Next Target - This class should handle all the moves in a generic manner
 * @date - 26/08/2018 - This class now handles moves of King as well 
 */
public class Moves {
	...
	/*
	 * This method will set the boundaries for king by initializing the last and
	 * first position
	 */
	public void initVarForKing() {
		if (alphaP != lastCharOnBoard) {
			lastCharOnBoard = (char) (alphaP + 1);
		}
		if (digitP != lastDigitOnBoard) {
			lastDigitOnBoard = digitP + 1;
		}
		if (alphaP != firstCharOnBoard) {
			firstCharOnBoard = (char) (alphaP - 1);
		}
		if (digitP != firstDigitOnBoard) {
			firstDigitOnBoard = digitP - 1;
		}
	...
	private Set<String> moveUpwardRightDiagonal(char alphaP, Set<String> addPosibilePosition) {
		if (tempD >= firstDigitOnBoard && tempD <= lastDigitOnBoard && alphaP >= firstCharOnBoard
				&& alphaP <= lastCharOnBoard) {
			tempD = tempD - 1;
			tempC = (char) (tempC + 1);
			if (tempC >= firstCharOnBoard && tempC <= lastCharOnBoard && tempD > firstDigitOnBoard - 1
					&& tempD < lastDigitOnBoard + 1) {
	 ...
	 * 
	 * @returns set of possible moves by appending all the moves from 1.
	 * moveUpwardLeftDiagonal 2. moveDownwardRightDiagonal 3.
	 * moveDownwardLeftDiagonal
	 */
	 ...
	 * 
	 * @return set of possible moves by appending the below combination 1. only
	 * horizontal 2. horizontal + diagonal (except upward right diagonal) 3.
	 * diagonal (except upward right diagonal)
	 */
	public Set<String> moveHorizontalDiagonalExceptUpwardRight(Piece pieceType, Set<String> addPosibilePosition) {
		for (char a = (char) (alphaP - (char) (alphaP - 1)); a <= lastCharOnBoard; a++) {
			if (a >= firstCharOnBoard && a <= lastCharOnBoard) {
				if (pieceType.getPieceType().equals("Queen")) {
					...
					// diagonal
					addPosibilePosition.addAll(moveInAllDiagonalExceptUpwardRightDiagonal(addPosibilePosition, a));
				} else if (pieceType.getPieceType().equals("King")) {
					// Horizontal Movement
					addPosibilePosition.add(String.valueOf(a) + String.valueOf(digitP));
					// Move in all diagonal Positions except upward Right
					// diagonal
	 ...
	 * 
	 * @returns appended list of 1. vertical movement 2. diagonal movement
	 */
	public Set<String> moveVerticalOrDiagonalWithUpwardRight(Piece pieceType, Set<String> addPosibilePosition) {
		if (digitP != firstDigitOnBoard) {
		...
		} else if (!pieceType.getPieceType().equals("Bishop") || !pieceType.getPieceType().equals("Pawn")) {
